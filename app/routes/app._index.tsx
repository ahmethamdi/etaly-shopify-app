...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  try {
    ...
    // Mock analytics data (will be replaced with real analytics later)
    const stats = {
      etaShownToday: 0,
      etaShownChange: null,
      productPagesUsingETA: 0,
      productPagesChange: null,
      cartPagesUsingETA: 0,
      cartPagesChange: null,
      conversionUplift: "0%",
      conversionChange: "No data yet",
    };

    // Transform delivery rules
    ...
    // Count rules by carrier
    const allRules = await db.deliveryRule.findMany({
    ...
    allRules.forEach((rule) => {
      const carrier = rule.carrier || "Custom";
      carrierCounts[carrier] = (carrierCounts[carrier] || 0) + 1;
    });

    const shippingMethods = Object.entries(carrierCounts).map(([name, count]) => ({
      name,
      rulesCount: count,
    }));
    ...
    return {
      ...
      deliveryRules,
      shippingMethods,
      hasRules: deliveryRules.length > 0,
    };
  } catch (error) {
...
export default function Dashboard() {
  const { shop, stats, deliveryRules, shippingMethods, hasRules } = useLoaderData<typeof loader>();
    ...
    <div style={{ padding: "24px", background: "#f9fafb", minHeight: "100vh" }}>
      ...
      <div style={{ marginBottom: "24px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
          ...
          <Button variant="primary" onClick={() => navigate("/app/delivery-rules")}>Create Delivery Rule</Button>
        </div>
      </div>
        ...
        <Card>
          <div style={{ padding: "20px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
              ...
              <Button variant="plain" onClick={() => navigate("/app/delivery-rules")}>View all</Button>
            </div>
              ...
              {!hasRules ? (
                <div style={{ textAlign: "center", padding: "40px 20px" }}>
                  <Text as="p" variant="bodyMd" tone="subdued">
                    No delivery rules yet. Create your first rule to start showing delivery estimates!
                  </Text>
                  <div style={{ marginTop: "16px" }}>
                    <Button onClick={() => navigate("/app/delivery-rules")}>Create Your First Rule</Button>
                  </div>
                </div>
              ) : (
                ...
                >
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                      ...
                      <Button variant="plain" size="slim" onClick={() => navigate("/app/delivery-rules")}>Edit</Button>
                    </div>
                  </div>
                </div>
              )))}
